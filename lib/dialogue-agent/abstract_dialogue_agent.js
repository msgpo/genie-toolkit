...
const { contactSearch } = require('./entity-linking/contact_search');
 ...
 */
module.exports = class AbstractDialogueAgent {
    ...
    async _chooseDevice(selector, hints) {
        ...
        if (name !== undefined)
            selecteddevices = alldevices.filter((d) => like(d.name, name.toJS()));
        ...
        const choice = await this.disambiguate('device',
            selecteddevices.length && name ? name.toJS() : null, choosefrom.map((d) => d.name), kind);
    ...
    async _addDisplayToDevice(value) {
        ...
        return classDef.metadata.thingpedia_name || classDef.metadata.canonical ||
    ...
    async _concretizeValue(slot, hints) {
        ...
        if (value.isMeasure && value.unit.startsWith('default')) {
            ...
            const candidates = (hints.idEntities ? hints.idEntities.get(value.type) : undefined)
                || await this.lookupEntityCandidates(value.type, value.display);
